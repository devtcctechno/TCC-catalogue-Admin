...
import { ITEM_LENGTH_ADD, ITEM_LENGTH_DELETE, ITEM_LENGTH_EDIT, ITEM_LENGTH_GET_ALL, ITEM_LENGTH_STATUS } from 'src/services/AdminServices'
...
const Itemlength = () => {
    ...
    const editApi = async (data: any) => {
        ...
        try {
            const data = await ITEM_LENGTH_EDIT(id, payload);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                toggleAddItemLenghDrawer();
                clearFormDataHandler();
    ...
    const deleteApi = async () => {
        try {
            const data = await ITEM_LENGTH_DELETE(id);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel);
                getAllApi(pagination);
    ...
    const statusApi = async (checked: boolean, row: any) => {
        try {
            const data = await ITEM_LENGTH_STATUS(row.id);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                getAllApi(pagination);
            } else {
