...
const SettingCaratWeight = () => {
  ...
  const editApi = async (data: any) => {
    ...
    try {
      const data = await SETTING_WEIGHT_EDIT(settingCaratWeightId, payload);
  ...
  const deleteApi = async () => {
    try {
      const data = await SETTING_WEIGHT_DELETE(settingCaratWeightId);
  ...
  const statusApi = async (checked: boolean, row: any) => {
    try {
      const data = await SETTING_WEIGHT_STATUS(row.id);
