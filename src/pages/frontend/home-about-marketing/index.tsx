...
import { TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_ADD, TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_DELETE, TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_EDIT, TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_GET_ALL, TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_STATUS } from 'src/services/AdminServices'

const HomeAboutMarketingSection = () => {
    let timer: any;
    ...
    const getAllBannerDataApi = async (bPagination: ICommonPagination) => {
        try {
            const data = await TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_GET_ALL(bPagination);
    ...
    const addBannerApi = async (data: any) => {
        if (!imageFile) {
            ...
            try {
                const data = await TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_ADD(formData)
                if (data.code === 200 || data.code === "200") {
    ...
    const editBannerDataApi = async (data: any) => {
        ...
        formData.append("content", data.content)
        try {
            const data = await TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_EDIT(formData)
            if (data.code === 200 || data.code === "200") {
    ...
    const deleteBannerDataApi = async () => {
        ...
        try {
            const data = await TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_DELETE(payload);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel)
                getAllBannerDataApi(pagination)
    ...
    const activeStatusDataApi = async (checked: boolean, row: any) => {
        ...
        try {
            const datas = await TEMPLATE_TWO_HOME_ABOUT_MARKETING_SECTION_STATUS(payload)
    ...
    const handleChangePerPageRows = (perPageRows: number) => {
        getAllBannerDataApi({ ...pagination, per_page_rows: perPageRows })
    }
    ...
    return (
        <Grid container spacing={6} sx={{ marginTop: 4 }}>
            <Grid item xs={12}>
                <Card>
                    <CardHeader title='Home About Section Banner'></CardHeader>
                    <Divider />
                    <Box>
                        <TCCTableHeader isButton value={searchFilter}
                            ...
                            ButtonName='Add Home About Marketing Section'
                        />
...
export default HomeAboutMarketingSection