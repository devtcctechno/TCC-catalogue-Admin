...
const Taxs = () => {
    ...
    const handleChangePerPageRows = (perPageRows: any) => {
        taxGetallApi({ ...pagination, per_page_rows: perPageRows.pageSize, current_page: perPageRows.page + 1 })
    }
    ...
    const taxEditApi = async (data: any) => {
        ...
        try {
            const data = await TAX_EDIT(id, payload);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                toggleAddTaxDrawer();
                clearFormDataHandler();
    ...
    const taxDeleteApi = async () => {
        ...
        try {
            const data = await TAX_DELETE(id);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel);
                taxGetallApi(pagination);
    ...
    const taxStatusApi = async (checked: boolean, row: any) => {
        try {
            const data = await TAX_STATUS(row.id);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                taxGetallApi(pagination);
