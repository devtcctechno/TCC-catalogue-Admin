...
const State = () => {
    ...
    const handleChangePerPageRows = (perPageRows: any) => {
        getAllStateApi({ ...pagination, per_page_rows: perPageRows.pageSize, current_page: perPageRows.page + 1 })
    }
    ...
    const editApi = async (data: any) => {
        ...
        try {
            const data = await STATE_EDIT(stateId, payload);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                toggleAddStateDrawer();
                clearFormDataHandler();
    ...
    const deleteApi = async () => {
        try {
            const data = await STATE_DELETE(stateId);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel);
                getAllStateApi(pagination);
    ...
    const statusApi = async (checked: boolean, row: any) => {
        try {
            const data = await STATE_STATUS(row.id);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                getAllStateApi(pagination);
