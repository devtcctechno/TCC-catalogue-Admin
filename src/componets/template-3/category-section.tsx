...
import { ADD_PRODUCT_DROPDOWN_LIST, TEMPLATE_THREE_BANNER_ADD, TEMPLATE_THREE_BANNER_DELETE, TEMPLATE_THREE_BANNER_EDIT, TEMPLATE_THREE_BANNER_GET_ALL, TEMPLATE_THREE_BANNER_STATUS, TEMPLATE_THREE_CATEGORY_SECTION_ADD, TEMPLATE_THREE_CATEGORY_SECTION_DELETE, TEMPLATE_THREE_CATEGORY_SECTION_EDIT, TEMPLATE_THREE_CATEGORY_SECTION_GET_ALL, TEMPLATE_THREE_CATEGORY_SECTION_STATUS, TEMPLATE_TWO_BANNER_ADD, TEMPLATE_TWO_BANNER_DELETE, TEMPLATE_TWO_BANNER_EDIT, TEMPLATE_TWO_BANNER_GET_ALL, TEMPLATE_TWO_BANNER_STATUS } from '../../services/AdminServices'
...
const CategorySection = () => {
    ...
    const getAllBannerDataApi = async (bPagination: ICommonPagination) => {
        try {
            const data = await TEMPLATE_THREE_CATEGORY_SECTION_GET_ALL(bPagination);
            if (data.code === 200 || data.code === "200") {
    ...
    const addBannerApi = async (data: any) => {
        if (!imageFile) {
            ...
            try {
                const data = await TEMPLATE_THREE_CATEGORY_SECTION_ADD(formData)
                if (data.code === 200 || data.code === "200") {
                    toggleAddBannerDrawer();
    ...
    const editBannerDataApi = async (data: any) => {
        ...
        try {
            const data = await TEMPLATE_THREE_CATEGORY_SECTION_EDIT(bannerId, formData)
            if (data.code === 200 || data.code === "200") {
                getAllBannerDataApi(pagination)
                toggleAddBannerDrawer();
                clearFormData();
    ...
    const deleteBannerDataApi = async () => {
        try {
            const data = await TEMPLATE_THREE_CATEGORY_SECTION_DELETE(bannerId);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel)
                getAllBannerDataApi(pagination)
    ...
    const activeStatusDataApi = async (checked: boolean, row: any) => {
        try {
            const datas = await TEMPLATE_THREE_CATEGORY_SECTION_STATUS(row.id)
    ...
    return (
        <Grid container spacing={6} sx={{ mt: 4 }}>
            <Grid item xs={12}>
                <Card>
                    <CardHeader title='Category Section'></CardHeader>
                    <Divider />
                    <Box>
                        <TCCTableHeader isButton value={searchFilter}
                            ...
                            ButtonName='Add Category Section'
                        />
            ...
            >
                ...
                <DrawerHeader
                    title={`${dialogTitle} Category Section`}
                    onClick={() => {
                        clearFormData()
                        toggleAddBannerDrawer()
                    }}
