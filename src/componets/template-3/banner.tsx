...
import { TEMPLATE_THREE_BANNER_ADD, TEMPLATE_THREE_BANNER_DELETE, TEMPLATE_THREE_BANNER_EDIT, TEMPLATE_THREE_BANNER_GET_ALL, TEMPLATE_THREE_BANNER_STATUS, TEMPLATE_TWO_BANNER_ADD, TEMPLATE_TWO_BANNER_DELETE, TEMPLATE_TWO_BANNER_EDIT, TEMPLATE_TWO_BANNER_GET_ALL, TEMPLATE_TWO_BANNER_STATUS } from '../../services/AdminServices'
...
const Banner = () => {
    ...
    const getAllBannerDataApi = async (bPagination: ICommonPagination) => {
        try {
            const data = await TEMPLATE_THREE_BANNER_GET_ALL(bPagination);
    ...
    const addBannerApi = async (data: any) => {
        if (!imageFile) {
            ...
            try {
                const data = await TEMPLATE_THREE_BANNER_ADD(formData)
                if (data.code === 200 || data.code === "200") {
                    toggleAddBannerDrawer();
    ...
    const editBannerDataApi = async (data: any) => {
        ...
        formData.append("sort_order", sortOrder)
        try {
            const data = await TEMPLATE_THREE_BANNER_EDIT(bannerId, formData)
            if (data.code === 200 || data.code === "200") {
                getAllBannerDataApi(pagination)
                toggleAddBannerDrawer();
                clearFormData();
    ...
    const deleteBannerDataApi = async () => {
        try {
            const data = await TEMPLATE_THREE_BANNER_DELETE(bannerId);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel)
                getAllBannerDataApi(pagination)
    ...
    const activeStatusDataApi = async (checked: boolean, row: any) => {
        ...
        try {
            const datas = await TEMPLATE_THREE_BANNER_STATUS(row.id)
