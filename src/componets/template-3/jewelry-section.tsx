...
const JewelrySection = () => {
    ...
    const [imageContentFile, setImageContentFile] = useState<string>()
    const [imageContentShow, setImageContentShow] = useState("")
    const [removeContentimage, setRemoveContentImage] = useState("0")
    const [imageContentValue, setImageContentValue] = useState('')
    const [collectionData, setCollectionData] = useState<any>([])
    const [collectionId, setCollectionId] = useState("")
    const [pagination, setPagination] = useState({ ...createPagination(), search_text: "" });
    console.log("imageTwoShow", imageTwoShow);

    const toggleAddBannerDrawer = () => {
        if (drawerAction == false) {
            setRemoveImage("1")
            setRemoveTwoImage("1")
            setRemoveContentImage("1")
        } else {
            setRemoveImage("0")
            setRemoveTwoImage("0")
            setRemoveContentImage("0")
        }
    ...
    const editOnClickHandler = (data: any) => {
        setImageContentShow(data.title_image_path)
    ...
    const getAllBannerDataApi = async (bPagination: ICommonPagination) => {
        try {
            const data = await TEMPLATE_THREE_JEWELRY_SECTION_GET_ALL(bPagination);
            if (data.code === 200 || data.code === "200") {
    ...
    const addBannerApi = async (data: any) => {
        if (!imageFile) {
            ...
            formData.append("title_image", imageContentFile || "")
            ...
            formData.append("button_colors", buttonColor)
            formData.append("sort_order", sortOrder)
            formData.append("id_collection", collectionId)

            try {
                const data = await TEMPLATE_THREE_JEWELRY_SECTION_ADD(formData)
                if (data.code === 200 || data.code === "200") {
                    toggleAddBannerDrawer();
    ...
    const editBannerDataApi = async (data: any) => {
        ...
        formData.append("id_collection", collectionId)
        try {
            const data = await TEMPLATE_THREE_JEWELRY_SECTION_EDIT(bannerId, formData)
            if (data.code === 200 || data.code === "200") {
                getAllBannerDataApi(pagination)
                toggleAddBannerDrawer();
                clearFormData();
    ...
    const deleteBannerDataApi = async () => {
        try {
            const data = await TEMPLATE_THREE_JEWELRY_SECTION_DELETE(bannerId);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel)
                getAllBannerDataApi(pagination)
    ...
    const activeStatusDataApi = async (checked: boolean, row: any) => {
        try {
            const datas = await TEMPLATE_THREE_JEWELRY_SECTION_STATUS(row.id)
    ...
    return (
        <Grid container spacing={6} sx={{ mt: 4 }}>
            <Grid item xs={12}>
                <Card>
                    <CardHeader title='Jewelry Section'></CardHeader>
                    <Divider />
                    <Box>
                        <TCCTableHeader isButton value={searchFilter}
                            ...
                            ButtonName='Add Jewelry Section'
                        />
            ...
            >
                ...
                <DrawerHeader
                    title={`${dialogTitle} Jewelry Section`}
                    onClick={() => {
                        clearFormData()
                        toggleAddBannerDrawer()
                    }}
                ...
                <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        />
                        <Typography variant='body2' sx={{ mb: 2 }}>Main Image : </Typography>
                        ...
                        <Typography variant='body2' sx={{ mt: 4, mb: 2 }}>Content Image : </Typography>
                        <TccSingleFileUpload imageValue={setImageContentValue} onDrop={setImageContentFile} onClick={removeContentimage} imageFile={imageContentShow} />
