...
import { TEMPLATE_TWO_ADD_FEATURES_SECTION, TEMPLATE_TWO_DELETE_FEATURES_SECTION, TEMPLATE_TWO_EDIT_FEATURES_SECTION, TEMPLATE_TWO_GET_ALL_FEATURES_SECTION, TEMPLATE_TWO_STATUS_UPDATE_FEATURES_SECTION } from 'src/services/AdminServices'
...
const FeaturesSection = () => {
    ...
    const editOnClickHandler = async (data: any) => {
        ...
        setValue("link", data.target_url)
        setValue("buttonName", data.button_name)
    ...
    const getAllFeatureDataApi = async (mbPagination: ICommonPagination) => {
        try {
            const data = await TEMPLATE_TWO_GET_ALL_FEATURES_SECTION(mbPagination);
    ...
    const addFeatureApi = async (data: any) => {
        if (!imageFile) {
            ...
            formData.append("target_url", data.link)
            formData.append("button_name", data.buttonName)
            formData.append("sort_order", data.sortOrder)
            try {
                const data = await TEMPLATE_TWO_ADD_FEATURES_SECTION(formData)
                if (data.code === 200 || data.code === "200") {
                    toggleAddFeatureSection();
                    clearFormData();
                    deafultDateData();
    ...
    const editFeatureDataApi = async (data: any) => {
        ...
        formData.append("button_name", data.buttonName)
        try {
            const data = await TEMPLATE_TWO_EDIT_FEATURES_SECTION(formData)
            if (data.code === 200 || data.code === "200") {
    ...
    const deleteFeatureDataApi = async () => {
        ...
        try {
            const data = await TEMPLATE_TWO_DELETE_FEATURES_SECTION(payload);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel)
                getAllFeatureDataApi(pagination)
    ...
    const activeStatusDataApi = async (checked: boolean, row: any) => {
        ...
        try {
            const datas = await TEMPLATE_TWO_STATUS_UPDATE_FEATURES_SECTION(payload)
    ...
    const handleChangePerPageRows = (perPageRows: any) => {
        getAllFeatureDataApi({ ...pagination, per_page_rows: perPageRows.pageSize, current_page: perPageRows.page + 1 })
    }
    ...
    return (
        <Grid container spacing={6} sx={{ marginTop: 4 }}>
            <Grid item xs={12}>
                <Card>
                    <CardHeader title='Family Values & Traditions Data'></CardHeader>
                    <Divider />
                    <TCCTableHeader
                        isButton
                        value={searchFilter}
                        onChange={(e: any) => setSearchFilter(e.target.value)}
                        ButtonName='Add Family Values & Traditions Data'
                        toggle={() => {
                            setDialogTitle('Add')
                            toggleAddFeatureSection()
                            clearFormData()
            ...
            >
                ...
                <DrawerHeader
                    title={`${dialogTitle} Family Values & Traditions Data`}
                    onClick={() => {
                        toggleAddFeatureSection()
                        clearFormData()
                    }}
                ...
                <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <FormControl fullWidth sx={{ mb: 4 }}>
                            ...
                            />
                            {errors.link && <FormHelperText sx={{ color: 'error.main' }}>{FIELD_REQUIRED}</FormHelperText>}
                        </FormControl>
                        <FormControl fullWidth sx={{ mb: 4 }}>
                            <Controller
                                name='buttonName'
                                ...
                                render={({ field }: any) => (
                                    <TextField
                                        ...
                                        error={Boolean(errors.buttonName)}
                                        {...field}
                                    />
                                )}
                            />
                            {errors.buttonName && <FormHelperText sx={{ color: 'error.main' }}>{FIELD_REQUIRED}</FormHelperText>}
                        </FormControl>
                        <FormControl fullWidth sx={{ mb: 4 }}>
                            <Controller
                                name='sortOrder'
                        ...
                        <TccSingleFileUpload
                            imageValue={setImageValue} onDrop={setImageFile} onClick={removeimage} imageFile={imageShow} />
