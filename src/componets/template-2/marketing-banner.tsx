...
import { TEMPLATE_TWO_ADD_MARKETING_BANNER, TEMPLATE_TWO_DELETE_MARKETING_BANNER, TEMPLATE_TWO_EDIT_MARKETING_BANNER, TEMPLATE_TWO_GET_ALL_MARKETING_BANNER, TEMPLATE_TWO_STATUS_UPDATE_MARKETING_BANNER } from 'src/services/AdminServices'
...
const MarketingBanner = () => {
    ...
    const getAllBannerDataApi = async (mbPagination: ICommonPagination) => {
        try {
            const data = await TEMPLATE_TWO_GET_ALL_MARKETING_BANNER(mbPagination);
    ...
    const addBannerApi = async (data: any) => {
        if (!imageFile) {
            ...
            try {
                const data = await TEMPLATE_TWO_ADD_MARKETING_BANNER(formData)
                if (data.code === 200 || data.code === "200") {
                    toggleAddMarketingBanner();
                    clearFormData();
    ...
    const editBannerDataApi = async (data: any) => {
        ...
        formData.append("target_url_2", bannerLinkTwo)
        try {
            const data = await TEMPLATE_TWO_EDIT_MARKETING_BANNER(formData)
            if (data.code === 200 || data.code === "200") {
    ...
    const deleteBannerDataApi = async () => {
        ...
        try {
            const data = await TEMPLATE_TWO_DELETE_MARKETING_BANNER(payload);
            if (data.code === 200 || data.code === "200") {
                toast.success(data.message);
                setShowModel(!showModel)
                clearFormData()
    ...
    const activeStatusDataApi = async (checked: boolean, row: any) => {
        ...
        try {
            const datas = await TEMPLATE_TWO_STATUS_UPDATE_MARKETING_BANNER(payload)
    ...
    return (
        <Grid container spacing={6} sx={{ marginTop: 4 }}>
            <Grid item xs={12}>
                <Card>
                    <CardHeader title='Home Page Our Mission Data'></CardHeader>
                    <Divider />
                    <TCCTableHeader
                        isButton
                        value={searchFilter}
