...
// ** Configs
import { MENU_APP_NAME_COLOR, NAV_HEADER_LOGO_HEIGHT, NAV_HEADER_LOGO_WIDTH } from 'src/AppConfig'
...
const VerticalNavHeader = (props: Props) => {
  ...
  const conditionalColors = () => {
    if (mode === 'semi-dark') {
      ...
      return {
        '& .MuiTypography-root, & .MuiIconButton-root': {
          color: `rgba(${MENU_APP_NAME_COLOR})`
        }
      }
    }
  }
  ...
  return (
    <MenuHeaderWrapper className='nav-header' sx={{ pl: menuHeaderPaddingLeft(), ...conditionalColors() }}>
      {userNavMenuBranding ? (
        ...
        <LinkStyled href='/'>
          <img width={NAV_HEADER_LOGO_WIDTH} height={NAV_HEADER_LOGO_HEIGHT} src={`${process.env.NEXT_PUBLIC_IMG_ENDPOINT}/${companyInfo?.dark_image_path}`} alt={""} />

          {/* <HeaderTitle variant='h6' sx={{ ...menuCollapsedStyles, ...(navCollapsed && !navHover ? {} : { ml: 2.5 }) }}>
            {finalKeyData?.company_name}
          </HeaderTitle> */}
        </LinkStyled>
      )}
