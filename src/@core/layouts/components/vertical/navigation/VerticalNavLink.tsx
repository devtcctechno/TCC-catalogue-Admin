...
import { MENU_SELCTED_ITEM_COLOR, MENU_TITLE_COLOR } from 'src/AppConfig'
...
>(({ theme }) => ({
  ...
  '&.active': {
    '&, &:hover': {
      boxShadow: `0px 2px 6px ${hexToRGBA(MENU_SELCTED_ITEM_COLOR, 0.48)}`,
      background: `linear-gradient(72.47deg, ${theme.direction === 'ltr' ? MENU_SELCTED_ITEM_COLOR : hexToRGBA(MENU_SELCTED_ITEM_COLOR, 0.7)
        } 22.16%, ${theme.direction === 'ltr' ? hexToRGBA(MENU_SELCTED_ITEM_COLOR, 0.7) : MENU_SELCTED_ITEM_COLOR
        } 76.47%)`,
      '&.Mui-focusVisible': {
  ...
  const conditionalColors = () => {
    if (mode === 'semi-dark') {
      ...
      return {
        ...
        '& .MuiTypography-root, & svg': {
          color: `rgba(${MENU_TITLE_COLOR}, 0.6)`
        }
      }
  }
